    ...
    <div className="flex h-full w-full flex-col overflow-hidden bg-canvas">
      ...
      <div className="flex flex-1 items-center justify-around gap-4 px-12 py-8">
        {stats.map((stat, i) => (
          <div
            key={i}
            className="flex flex-1 flex-col items-center gap-2 rounded-xl border border-primary/15 bg-background-dark/60 px-6 py-8"
          >
            <span
            ...
            {stat.label && (
              <span className="text-center text-sm font-medium uppercase tracking-wider text-slate-400">
                {stat.label}
              </span>
            )}
          </div>
